...
@Component
public class SearchEmployeesByPositionList implements Input {
    private final AnswerConsumerImpl answerConsumer;
    private final EmployeeService employeeService;
    private final SubmittingAdditionalMessage message;
    @Autowired
    public SearchEmployeesByPositionList(AnswerConsumerImpl answerConsumer, EmployeeService employeeService,
    ...
    public void handle(Update update) {
        ...
        try {
            Employee employee = employeeService.searchEmployeeByProject(messageText).get(0);
