...
@Component
public class ChangeProjectChooseCallback implements CallbackHandler{
    private final SubmittingAdditionalMessage message;
    @Autowired
    public ChangeProjectChooseCallback(SubmittingAdditionalMessage message) {
    ...
    public void apply(Callback callback, Update update) {
        ...
        StateForEmployeeData.stateAndCard.put(BotInputState.WAITING_FOR_EMPLOYEE_DATA_FOR_EDIT_PROJECT, stateForEmployeeData.getValue());

        //return new SendMessage(chatId.toString(), "Введите проект:");
        message.sendMessage(new SendMessage(chatId.toString(), "Введите проект:"));
    }
}
