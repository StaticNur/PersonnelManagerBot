...
@Component
public class ChangeNameChooseCallback implements CallbackHandler{
    private final SubmittingAdditionalMessage message;
    @Autowired
    public ChangeNameChooseCallback(SubmittingAdditionalMessage message) {
    ...
    public void apply(Callback callback, Update update) {
        ...
        StateForEmployeeData.stateAndCard.put(BotInputState.WAITING_FOR_EMPLOYEE_DATA_FOR_EDIT_NAME, stateForEmployeeData.getValue());

        //return new SendMessage(chatId.toString(), "Введите имя:");
        message.sendMessage(new SendMessage(chatId.toString(), "Введите имя:"));
    }
}
