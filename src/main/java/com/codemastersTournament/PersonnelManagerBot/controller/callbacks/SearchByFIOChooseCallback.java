...
@Component
public class SearchByFIOChooseCallback implements CallbackHandler{
    private final AnswerConsumerImpl answerConsumer;
    private final SubmittingAdditionalMessage message;
    @Autowired
    public SearchByFIOChooseCallback(AnswerConsumerImpl answerConsumer, SubmittingAdditionalMessage message) {
    ...
    public void apply(Callback callback, Update update) {
        ...
        StateForEmployeeData.stateAndCard.put(BotInputState.WAITING_FOR_EMPLOYEE_DATA_FOR_SEARCH_FIO,new Employee());
