...
@Component
public class SearchByProjectChooseCallback implements CallbackHandler{
    private final AnswerConsumerImpl answerConsumer;
    private final SubmittingAdditionalMessage message;
    @Autowired
    public SearchByProjectChooseCallback(AnswerConsumerImpl answerConsumer, SubmittingAdditionalMessage message) {
    ...
    public void apply(Callback callback, Update update) {
        ...
        StateForEmployeeData.stateAndCard.put(BotInputState.WAITING_FOR_EMPLOYEE_DATA_FOR_SEARCH_BY_PROJECT,new Employee());
