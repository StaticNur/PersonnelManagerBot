...
@Component
public class SearchByPositionChooseCallback implements CallbackHandler{
    private final AnswerConsumerImpl answerConsumer;
    private final SubmittingAdditionalMessage message;
    @Autowired
    public SearchByPositionChooseCallback(AnswerConsumerImpl answerConsumer, SubmittingAdditionalMessage message) {
    ...
    public void apply(Callback callback, Update update) {
        ...
        StateForEmployeeData.stateAndCard.put(BotInputState.WAITING_FOR_EMPLOYEE_DATA_FOR_SEARCH_BY_POSITION,new Employee());
